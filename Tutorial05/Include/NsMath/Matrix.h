...
public:
    ...
    /// Accessors
    //@{
    inline Vector2<T>& operator[](NsSize i)
    {
        NS_ASSERT(i < 2);
        return mVal[i];
    }
    inline const Vector2<T>& operator[](NsSize i) const
    {
        NS_ASSERT(i < 2);
        return mVal[i];
    }
    ...
    /// Logic operators
    //@{
    inline NsBool operator==(const Matrix2& m) const
    {
...
private:
    ...
    NS_DECLARE_REFLECTION(Matrix2, Core::NoParent)
};
...
/// Decompose a non-degenerate 2x2 transformation matrix into the sequence of 
/// transformations that produced it:
///     m = [Sx][Sy][ShearXY][Rot]
template<class T> void Decompose(const Matrix2<T>& m, T& scaleX, T& scaleY, T& shearXY, T& rot);
...
public:
    ...
    /// Accessors
    //@{
    inline Vector3<T>& operator[](NsSize i)
    {
        NS_ASSERT(i < 3);
        return mVal[i];
    }
    inline const Vector3<T>& operator[](NsSize i) const
    {
        NS_ASSERT(i < 3);
        return mVal[i];
    }
    ...
    /// Logic operators
    //@{
    inline NsBool operator==(const Matrix3& m) const
    {
    ...
    static Matrix3 EulerZXY(T x, T y, T z);
    //@}
...
private:
    ...
    NS_DECLARE_REFLECTION(Matrix3, Core::NoParent)
};
...
template<class T> NsBool IsAffine(const Matrix3<T>& m);
...
/// Decompose a non-degenerate 3x3 transformation matrix into the sequence of 
/// transformations that produced it:
///     m = [SX][SY][SZ][ShearXY][ShearXZ][ShearZY][RX][RY][RZ]]
/// Based on http://www1.acm.org/pubs/tog/GraphicsGems/gemsii/unmatrix.c
template<class T> void Decompose(const Matrix3<T>& m, T& scaleX, T& scaleY, T& scaleZ, T& shearXY, 
    T& shearXZ, T& shearZY, T& rotateX, T& rotateY, T& rotateZ);

/// Orthonormalize a matrix using Gram-Schmidt orthogonalization
template<class T> Matrix3<T> Orthonormalize(const Matrix3<T>& m);
...
template<class T> void ToEulerZXY(const Matrix3<T>& m, T& x, T& y, T& z);
//@}
...
typedef Transform3<NsFloat32> Transform3f;
...
public:
    ...
    /// Accessors
    //@{
    inline Vector4<T>& operator[](NsSize i)
    {
        NS_ASSERT(i < 4);
        return mVal[i];
    }
    inline const Vector4<T>& operator[](NsSize i) const
    {
        NS_ASSERT(i < 4);
        return mVal[i];
    }
    ...
    /// Logic operators
    //@{
    inline NsBool operator==(const Matrix4& m) const
    {
    ...
    /// Returns a symmetric perspective projection matrix
    /// \param width Width of the view volume at the near plane
    /// \param height Height of the view volume at the far plane
    /// \param zNear Z near plane
    /// \param zFar Z far plane
    static Matrix4 Perspective(T width, T height, T zNear, T zFar);
    ...
    /// \param zNear Z near plane
    /// \param zFar Z far plane
    static Matrix4 PerspectiveFov(T fovY, T aspect, T zNear, T zFar);
    ...
    /// \param zNear Minimum z-value of the view volume
    /// \param zFar Maximum z-value of the view volume
    static Matrix4 Perspective(T left, T right, T bottom, T top, T zNear, T zFar);
...
private:
    ...
    NS_DECLARE_REFLECTION(Matrix4, Core::NoParent)
};
...
template<class T> NsBool IsAffine(const Matrix4<T>& m);
...
typedef Matrix2<NsFloat32> Matrix2f;
typedef Matrix3<NsFloat32> Matrix3f;
typedef Matrix4<NsFloat32> Matrix4f;


template<> inline const NsChar* IdOf<Matrix2f>() { return "Matrix2f"; }
template<> inline const NsChar* IdOf<Matrix3f>() { return "Matrix3f"; }
template<> inline const NsChar* IdOf<Matrix4f>() { return "Matrix4f"; }


#define NS_TEMPLATE extern template NS_MATH_VECTORMATH_API

NS_TEMPLATE NsString Matrix2f::ToString() const;
NS_TEMPLATE NsUInt32 Matrix2f::GetHashCode() const;
NS_TEMPLATE void Decompose(const Matrix2f&, NsFloat32&, NsFloat32&, NsFloat32&, NsFloat32 &);
NS_TEMPLATE const Core::TypeClass* Matrix2f::StaticGetClassType(Core::T2T<Matrix2f>*);

NS_TEMPLATE NsString Matrix3f::ToString() const;
NS_TEMPLATE NsUInt32 Matrix3f::GetHashCode() const;
NS_TEMPLATE Matrix3f::Matrix3(const Transform2f&);
NS_TEMPLATE Matrix3f Matrix3f::EulerZXY(NsFloat32, NsFloat32, NsFloat32);
NS_TEMPLATE void Decompose(const Matrix3f&, NsFloat32&, NsFloat32&, NsFloat32&,
    NsFloat32&, NsFloat32&, NsFloat32&, NsFloat32&, NsFloat32&, NsFloat32&);
NS_TEMPLATE void ToEulerZXY(const Matrix3f&, NsFloat32&, NsFloat32&, NsFloat32&);
NS_TEMPLATE const Core::TypeClass* Matrix3f::StaticGetClassType(Core::T2T<Matrix3f>*);

NS_TEMPLATE NsString Matrix4f::ToString() const;
NS_TEMPLATE NsUInt32 Matrix4f::GetHashCode() const;
NS_TEMPLATE Matrix4f::Matrix4(const Transform3f&);
NS_TEMPLATE Matrix4f& Matrix4f::operator*=(const Matrix4f& m);
NS_TEMPLATE Matrix4f Matrix4f::Ortho(NsFloat32, NsFloat32, NsFloat32, NsFloat32);
NS_TEMPLATE Matrix4f Matrix4f::Ortho(NsFloat32, NsFloat32, NsFloat32, NsFloat32, NsFloat32,
    NsFloat32);
NS_TEMPLATE Matrix4f Matrix4f::Perspective(NsFloat32, NsFloat32, NsFloat32, NsFloat32);
NS_TEMPLATE Matrix4f Matrix4f::PerspectiveFov(NsFloat32, NsFloat32, NsFloat32, NsFloat32);
NS_TEMPLATE Matrix4f Matrix4f::PerspectiveFov(NsFloat32, NsFloat32, NsFloat32);
NS_TEMPLATE Matrix4f Matrix4f::Perspective(NsFloat32, NsFloat32, NsFloat32, NsFloat32, NsFloat32,
    NsFloat32);
NS_TEMPLATE Matrix4f Matrix4f::Viewport(NsFloat32, NsFloat32);
NS_TEMPLATE Matrix4f Inverse(const Matrix4f&);
NS_TEMPLATE Matrix4f Inverse(const Matrix4f&, NsFloat32);
NS_TEMPLATE NsFloat32 Determinant(const Matrix4f&);
NS_TEMPLATE const Core::TypeClass* Matrix4f::StaticGetClassType(Core::T2T<Matrix4f>*);
