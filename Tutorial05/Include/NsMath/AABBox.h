...
typedef AABBox2<NsFloat32> AABBox2f;
...
public:
    ...
    /// Copy constructor
    inline AABBox2(const AABBox2& v);
    template<class S> inline AABBox2(const AABBox2<S>& v);
    
    /// Constructor from minX, minY, maxX, maxY
    inline AABBox2(NsFloat32 minX, NsFloat32 minY, NsFloat32 maxX, NsFloat32 maxY);
    ...
    /// Assignment
    inline AABBox2& operator=(const AABBox2& v);
    ...
    /// [2] : (min,max)
    /// [3] : (max,max)
    inline Vector2<T> operator[](NsSize i) const;
    ...
    ///// Logic operators
    //@{
    inline NsBool operator==(const AABBox2& box) const;
    inline NsBool operator!=(const AABBox2& box) const;
    //@}
...
template<class T> NsBool Inside(const AABBox2<T>& box, const Vector2<T>& p);

/// Function to check if two bounding boxes overlap
template<class T> NsBool BoxesIntersect(const AABBox2<T>& box0, const AABBox2<T>& box1);
