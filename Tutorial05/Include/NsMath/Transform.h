...
public:
    ...
    /// Logic operators
    //@{
    inline NsBool operator==(const Transform2& m) const
    {
    ...
    //@{
    static NsBool TryParse(const NsChar* str, Transform2& output);
    static Transform2 Parse(const NsChar* str);
    //@}
...
private:
    ...
    NS_DECLARE_REFLECTION(Transform2, Core::NoParent)
};
...
/// Decompose a non-degenerate 3x3 affine matrix into the sequence of 
/// transformations that produced it:
///     m = [Sx][Sy][ShearXY][Rot][TX][TY]
template<class T> void Decompose(const Transform2<T>& m, T& scaleX, T& scaleY, T& shearXY, 
    T& rotate, T& transX, T& transY);
...
public:
    ...
    /// Logic operators
    //@{
    inline NsBool operator==(const Transform3& m) const
    {
...
private:
    ...
    NS_DECLARE_REFLECTION(Transform3, Core::NoParent)
};
...
/// Decompose a non-degenerate 4x4 affine matrix into the sequence of 
/// transformations that produced it:
///     m = [SX][SY][SZ][ShearXY][ShearXZ][ShearZY][RX][RY][RZ][TX][TY][TZ]]
/// Based on http://www1.acm.org/pubs/tog/GraphicsGems/gemsii/unmatrix.c
template<class T> void Decompose(const Transform3<T>& m, T& scaleX, T& scaleY, T& scaleZ, 
    T& shearXY, T& shearXZ, T& shearZY, T& rotateX, T& rotateY, T& rotateZ, T& transX, T& transY, 
    T& transZ);
...
template<> inline const NsChar* IdOf<Transform2f>() { return "Transform2f"; }
template<> inline const NsChar* IdOf<Transform3f>() { return "Transform3f"; }


#define NS_TEMPLATE extern template NS_MATH_VECTORMATH_API

NS_TEMPLATE NsBool Transform2f::TryParse(const NsChar*, Transform2&);
NS_TEMPLATE Transform2f Transform2f::Parse(const NsChar*);
NS_TEMPLATE NsString Transform2f::ToString() const;
NS_TEMPLATE NsUInt32 Transform2f::GetHashCode() const;
NS_TEMPLATE const Core::TypeClass* Transform2f::StaticGetClassType(Core::T2T<Transform2f>*);
NS_TEMPLATE void Decompose(const Transform2f&, NsFloat32&, NsFloat32&, NsFloat32&,
    NsFloat32&, NsFloat32&, NsFloat32&);

NS_TEMPLATE NsString Transform3f::ToString() const;
NS_TEMPLATE NsUInt32 Transform3f::GetHashCode() const;
NS_TEMPLATE const Core::TypeClass* Transform3f::StaticGetClassType(Core::T2T<Transform3f>*);
NS_TEMPLATE void Decompose(const Transform3f&, NsFloat32&, NsFloat32&, NsFloat32&,
    NsFloat32&, NsFloat32&, NsFloat32&, NsFloat32&, NsFloat32&, NsFloat32&, NsFloat32&, NsFloat32&,
    NsFloat32&);
