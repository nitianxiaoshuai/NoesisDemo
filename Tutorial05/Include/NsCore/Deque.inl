...
deque_node<T, Allocator, kNodeSize>::deque_node() :
    mBufferBegin(reinterpret_cast<T*>((NsUIntPtr(mBuffer) + (ALIGNMENT - 1)) & ~(ALIGNMENT - 1))),
...
{
    NS_ASSERT(mBegin != 0, "Invalid Node");
    NS_ASSERT(can_push_back(), "Can't push back");
...
{
    NS_ASSERT(mBegin != 0, "Invalid Node");
    NS_ASSERT(can_push_front(), "Can't push front");
...
{
    NS_ASSERT(mBegin != 0, "Invalid Node");
    NS_ASSERT(n < mSize, "Index out of range");
...
{
    NS_ASSERT(headPos < HeadPos_Count, "Invalid head pos");
...
{
    for (NsUInt32 i = 0; i < mVector.size(); ++i)
    {
...
{
    NS_ASSERT(mSize != 0, "Empty buffer");
...
{
    NS_ASSERT(mParent == other.mParent, "Invalid iterator comparation");
    return mOffset < other.mOffset;
}
