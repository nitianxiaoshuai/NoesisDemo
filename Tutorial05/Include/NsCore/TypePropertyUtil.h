...
//@{
template<class T>
inline T* PtrOffset(void* ptr, NsSize offset)
{
    return reinterpret_cast<T*>(static_cast<NsByte*>(ptr) + offset);
}

template<class T>
inline const T* PtrOffset(const void* ptr, NsSize offset)
{
    return reinterpret_cast<const T*>(static_cast<const NsByte*>(ptr) + offset);
}
//@}
...
{
    return Boxing::Box<NsString>(prop->Get<T>(ptr));
}

template<class T>
Ptr<BaseComponent> GetComponent(const TypeProperty* prop, const void* ptr, NonRefCountedType)
{
    enum { IsStringType = (int)IsSame<T, const NsChar*>::Result + 3 };
